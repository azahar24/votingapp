...
import '../ui/views/vote_screen.dart';

class MyElc extends StatelessWidget {
            ...
            children: [
              if (ongoingElections.isNotEmpty) ...[
                ...
                ...ongoingElections.map((election) => Card(
                      child: ListTile(
                        onTap: () {
                          Get.to(UpdateElection(elcName: election.name, startTimenav: DateFormat('yyyy-MM-dd HH:mm').format(election.startTime), endTimenav: DateFormat('yyyy-MM-dd HH:mm').format(election.endTime), docid: election.id));
                        },
                        title: Text(
          ...
          return ListView(
            children: [
              if (upcomingElections.isNotEmpty) ...[
                ...
                ...upcomingElections.map((election) => Card(
                      child: ListTile(
                        onTap: () {
                          Get.to(UpdateElection(elcName: election.name, startTimenav: DateFormat('yyyy-MM-dd HH:mm').format(election.startTime), endTimenav: DateFormat('yyyy-MM-dd HH:mm').format(election.endTime), docid: election.id));
                        },
                        ...
                        subtitle: Text(
                            'Starts at: ${DateFormat('yyyy-MM-dd HH:mm').format(election.startTime)}'),
                      ),
                    )),
              ],
            ],
